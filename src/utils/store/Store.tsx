...
export const saveGuestFlag = (isGuest: boolean) => {
  if (isGuest) {
    AsyncStorage.setItem(KEY_GUEST_FLAG, "guest");
  } else {
    AsyncStorage.setItem(KEY_GUEST_FLAG, "social_user");
  }
}
...
export const saveToken = (token: string) => AsyncStorage.setItem(KEY_TOKEN, token);
...
export const saveUuidUser = (uuid: string) => AsyncStorage.setItem(KEY_UUID, uuid);
